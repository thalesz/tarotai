...
@router.get(
    "/daily/{count}",
    summary="Receber último(s) horóscopo(s) diário(s) do zodíaco. Count define qual registro retornar. sendo 1 o mais recente ate o 7 o mais antigo",
    description="Retorna o(s) último(s) horóscopo(s) diário(s) do zodíaco para o usuário autenticado. O parâmetro 'count' define quantos registros retornar.",
    response_description="Horóscopo diário do zodíaco retornado com sucesso.",
    responses={
        200: {
...
async def get_daily_zodiac(
    ...
    try:
        ...
        daily_zodiac = await DailyZodiacSchemaBase.get_daily_zodiac_by_user_id(
            session=db,
            user_id=user_id,
            count=count
        )
        ...
        if not daily_zodiac:
            raise HTTPException(
                status_code=404,
                detail="Zodiaco diario não encontrado para o usuário."
            )

        reading = JsonExtractor.extract_json_from_reading(daily_zodiac.reading)
